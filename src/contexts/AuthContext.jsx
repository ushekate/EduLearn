...
export function AuthContextProvider({ children }) {
	const [user, setUser] = useState(pbstudent.authStore.record);
	...
	async function Login(emailOrUsername, password, role) {
		try {
			if (emailOrUsername !== '' || emailOrUsername !== null) {
				...
				const res = await pbstudent.collection('students').authWithPassword(identity, password);
	...
	async function Register(email, username, password, passwordConfirm, role) {
		try {
			...
			const res = await pbstudent.collection('students').create(data);
			return res;
		} catch (err) {
			throw new Error(err)
		}
